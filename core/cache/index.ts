...
import { readJsonFile, writeJsonFile } from 'core/fs-read-write-json'
...
export function getCached<V>(key: string, fn: () => Promise<V>, cacheDurationMs: number) {
  const tempFilePath = getTempPath(`./jgdev-cache-${sha(key)}.json`)

  const getEntry = async () => readJsonFile<CacheEntry<V> | undefined>(tempFilePath)
  ...
  const hasExpired = (entry: CacheEntry<V>) => new Date().getTime() > entry.expiresAt
